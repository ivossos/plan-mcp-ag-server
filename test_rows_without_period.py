"""Test rows without Period (since it's in columns)."""
...
async def test_rows_without_period():
    config = load_config()
    client = PlanningClient(config)
    
    # Period is in columns, so remove it from rows
    # Rows should have Account dimension (and maybe others, but not Period)
    grid_definition = {
    ...
    print("Testing rows with Account only (Period is in columns):")
...
if __name__ == "__main__":
    asyncio.run(test_rows_without_period())
